...
class Node:
	...
	def create(self,  operators=None, terminals=None, depth=None,full=False):
		if depth>1 and (random()<0.5 or full ==True ):
			op, n_args = operators[randint(0,len(operators)-1)]
			self.value = op

			self.branches = []
			for i in range(n_args):
				n = Node()
				n.create( operators, terminals, depth-1)
	...
	def getRandomNode(self,  value=None):
		...
		for i in range(len(self.branches)):
			...
			if value-1 < size:
				return self.branches[i].getRandomNode( value-1)
			value -= size
	...
	def calculate(self, sample):
		...
		if self.branches == None:
			...
			if self.value == "max": # max( X0, X1, ... Xn)
				...
				return np.array(a)
			if self.value == "log": # log2(X)
				res = self.branches[0].calculate(sample)
				res = np.where(res<=0, res, np.log(res))
				return res
			if self.value == "sqrt": # sqrt(X)
				res = self.branches[0].calculate(sample)
				res = np.where(res<=0, res, np.sqrt(res))
				return res
	...
	def getFeatures(self, lf):
		if self.left == None:
			lf = lf + [str(self.value)]
		else:
			lf = lf + (self.left.getFeatures(lf) + self.right.getFeatures(lf))
